...
function IndividualChat({ chat, chatPerson, currentProfile }) {
  ...
  const sendMessage = () => {
    ...
    const newLastChar =
      Number(lastMessageID.charAt(lastMessageID.length - 1)) + 1;
  ...
  return (
    <div className="flex flex-col max-w-md mx-auto my-5 shadow-lg h-96">
      <div className="text-white text-xl font-semibold pl-3">
        Chatting With: {chatPerson}
      </div>
      <div
      ...
      <div className="flex-none p-4 bg-orange-200">
        <input
          type="text"
        ...
        <button
          onClick={sendMessage}
          className="text-white bg-amber-500 rounded-lg px-4 py-2 pt-6"
        >
          Send
        </button>
      </div>
